...
In turn, ForEach-Object takes each server in the collection and runs the New-CsMcxConfiguration cmdlet in order to create new Mobility Service configuration settings on that server.
...
Example 3 demonstrates how the InMemory parameter enables you to create a new collection of Mobility Service configuration settings in memory, modify the property values of that collection, and then save the collection to Lync Server.
...
Note that if you do not call Set-CsMcxConfiguration, no settings will ever be assigned to the Redmond site and your virtual collection will disappear as soon as you exit your Windows PowerShell session or delete the variable $x.
...
Example 3 demonstrates how the InMemory parameter enables you to create a new collection of Mobility Service configuration settings in memory, modify the property values of that collection, and then save the collection Skype for Business Server 2015.
To do this, the first command in the collection creates a new collection of Mobility Service configuration settings that have the Identity site:Redmond.
However, instead of being automatically created and assigned to the Redmond site, these settings are created in memory only (because of the InMemory parameter) and are then stored in a variable named $x.
...
###  
None.
New-CsMcxConfiguration does not accept pipelined input.
...
[Online Version](http://technet.microsoft.com/EN-US/library/9eebea4a-64a3-424c-9b05-0579c4e8111e(OCS.15).aspx)

[Online Version](http://technet.microsoft.com/EN-US/library/9eebea4a-64a3-424c-9b05-0579c4e8111e(OCS.16).aspx)
