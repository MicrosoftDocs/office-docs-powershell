...
In that case you only need to specify the SIP address of a user, and Lync Server 2010 will use certificates to authenticate that user.

When you run Test-CsPstnOutboundCall, the cmdlet first attempts to log the test user on to Lync Server.
...
Test-CsPstnOutboundCall can also be used in server platform mode.
In that case you only need to specify the SIP address of a user, and Lync Server will use certificates to authenticate that user.
...
Test-CsPstnOutboundCall can also be used in server platform mode.
In that case you only need to specify the SIP address of a user, and Skype for Business Server 2015 will use certificates to authenticate that user.

When you run the Test-CsPstnOutboundCall cmdlet, the cmdlet first attempts to log the test user on to Skype for Business Server 2015.
If the logon succeeds, the cmdlet will then attempt to make a phone call across the PSTN gateway.
This phone call will be placed using the dial plan, voice policy, and other policies and settings assigned to the test account.
When the call is answered, the cmdlet sends dual-tone multi-frequency (DTMF) codes across the network in order to verify media connectivity.
...
The preceding example checks to see if a preconfigured test user can log on to the pool atl-cs-001.litwareinc.com and then make a phone call across the PSTN gateway.
This command will work only if test users been defined for the pool atl-cs-001.litwareinc.com.
If they have, then the command will determine whether the first test user i can log on to the system and, if so, make a phone call to a phone located on the PSTN network.
...
Test-CsPstnOutboundCall will then conduct its checks using the specified user.
...
The commands shown in Example 2 test the ability of a test user (litwareinc\kenmyer) to log on to Lync Server and then make a phone call over the PSTN gateway.
...
With the credential object in hand, the second command in the example determines whether or not the test user can log on to Lync Server, and then make a phone call to the target phone number (+15551234567).
To carry out this task, Test-CsPstnOutboundCall is called, along with the following parameters: TargetFqdn (the FQDN of the Registrar pool); UserSipAddress (the SIP address for the user making the call); UserCredential (the Windows PowerShell object containing the credentials for the test user); and TargetPstnPhoneNumber (the phone number being called).
...
The commands shown in Example 2 test the ability of a test user (litwareinc\kenmyer) to log on to Skype for Business Server 2015 and then make a phone call over the PSTN gateway.
To do this, the first command in the example uses the Get-Credential cmdlet to create a Windows PowerShell credential object containing the name and password of the user Ken Myer.
(Because the logon name litwareinc\kenmyer has been included as a parameter, the Windows PowerShell Credential Request dialog box only requires the administrator to enter the password for the Ken Myer account.) The resulting credentials object is then stored in a variable named $cred1.

With the credential object in hand, the second command in the example determines whether or not the test user can log on Skype for Business Server 2015, and then make a phone call to the target phone number (+15551234567).
...
Example 3 shows how Test-CsPstnOutboundCall can be used in server platform mode.
In this mode, the user's SIP address is specified, but the user credentials are not included.
When run like this, Lync Server 2010 uses certificates to authenticate the test user.
...
Example 3 shows how Test-CsPstnOutboundCall can be used in server platform mode.
In this mode, the user's SIP address is specified, but the user credentials are not included.
When run like this, Lync Server uses certificates to authenticate the test user.
...
In this mode, the user's SIP address is specified, but the user credentials are not included.
When run like this, Skype for Business Server 2015 uses certificates to authenticate the test user.
...
Lync Server 2010 will then use that dial plan to normalize the number to the E.164 format.
...
If you do not use the E.164 format the dial plan of the test user will be appended to the number.
Lync Server will then use that dial plan to normalize the number to the E.164 format.
If the number cannot be normalized then the call cannot be placed and the test will fail.
...
If you do not use the E.164 format the dial plan of the test user will be appended to the number.
Skype for Business Server 2015 will then use that dial plan to normalize the number to the E.164 format.
If the number cannot be normalized then the call cannot be placed and the test will fail.
...
In that case, Lync Server 2010 will attempt to authenticate the user by using certificates.
...
This parameter is not needed if the command is using test users configured by using the CsHealthMonitoringConfiguration cmdlets.
You also do not need to specify this parameter if the test is being conducted in server platform mode.
In that case, Lync Server will attempt to authenticate the user by using certificates.
...
This parameter is not needed if the command is using test users configured by using the CsHealthMonitoringConfiguration cmdlets.
You also do not need to specify this parameter if the test is being conducted in server platform mode.
In that case, Skype for Business Server 2015 will attempt to authenticate the user by using certificates.
...
###  
None.
Test-CsPstnOutboundCall does not accept pipelined input.
...
[Online Version](http://technet.microsoft.com/EN-US/library/12d940c3-8526-4c8f-8dc1-3fe65a3211bc(OCS.14).aspx)

[Test-CsPstnPeerToPeerCall]()

[Online Version](http://technet.microsoft.com/EN-US/library/12d940c3-8526-4c8f-8dc1-3fe65a3211bc(OCS.15).aspx)

[Online Version](http://technet.microsoft.com/EN-US/library/12d940c3-8526-4c8f-8dc1-3fe65a3211bc(OCS.16).aspx)
