...
- `IneligibleUser` - to query for a user that do not have valid Teams license (except Guest, ResourceAccount, SfbOnPremUser).
...
Sorting is now enabled in Teams PowerShell Module by using the "-Sort" or "-OrderBy" parameters in Commercial, GCC High and DoD environments. For example:
...
Commonly used FeatureTypes with the resepective definitions are shown below: 
