...
**Note**: The action of a rule without conditions or exception is applied to all messages, which could have unintended consequences. For example, if the rule action deletes messages, the might delete all inbound and outbound messages for the entire organization.
