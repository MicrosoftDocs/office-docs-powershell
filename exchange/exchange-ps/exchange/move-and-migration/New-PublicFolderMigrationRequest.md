...
The RemoteMailboxServerLegacyDN parameter specifies the server legacy distinguished name (DN) of the back-end server. To find the LegacyExchangeServerDN property, run the following command: `Get-ExchangeServer <ServerIdentity> | Format-List LegacyExchangeServerDN`.
...
- If the request fails, you need to manually remove it by using the corresponding RemoveRequest cmdlet.
