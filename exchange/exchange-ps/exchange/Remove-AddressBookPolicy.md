...
Use the Remove-AddressBookPolicy cmdlets to delete an address book policy. You can't remove the address book policy if it is still assigned to a user's mailbox.
...
You can't delete an address book policy if it is assigned to a user. To determine if an address book policy is assigned to a user, run the following command:
...
```
Remove-AddressBookPolicy -Identity "Mrs. Munger's Class"
```

This example deletes the address book policy Mrs. Munger's Class.
...
[Online Version](https://technet.microsoft.com/library/57ff215a-cba5-46d1-a7f7-ab2512ce4b6f.aspx)
