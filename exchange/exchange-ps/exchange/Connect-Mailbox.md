...
The AddressBookPolicy parameter specifies the Address book policy to apply to this mailbox when it's connected.
...
The ManagedFolderMailboxPolicyAllowed switch bypasses the warning that messaging records management (MRM) features aren't supported for email clients using versions of Microsoft Outlook earlier than Office Outlook 2007. When a managed folder mailbox policy is assigned to a mailbox using the ManagedFolderMailboxPolicy parameter, the warning appears by default unless the ManagedFolderMailboxPolicyAllowed switch is used.
...
The Force switch specifies whether to suppress warning or confirmation messages. This switch can be used when the task is run programmatically and prompting for administrative input is inappropriate. If the Force switch isn't provided in the command, administrative input is prompted. If the Force switch is provided in the command, but the value is omitted, its default value is $true.
