...
- Cutover Exchange migration: This is another type of onboarding migration and is used to migrate all mailboxes in an on-premises Exchange organization to Exchange Online. You can migrate a maximum of 2,000 Microsoft Exchange Server 2003, Exchange Server 2007, or Exchange Server 2010 mailboxes using a cutover migration. Mailboxes will be automatically provisioned in Exchange Online when you perform a cutover Exchange migration. For more information, see Example 5.
...
The UserIds parameter specifies a comma-separated list of user identities to be included in the migration batch.
...
The Users parameter specifies a comma-separated list of users to be included in the migration batch.
...
The CSVData parameter specifies the CSV file that contains information about the user mailboxes to be moved or migrated. The required attributes in the header row of the CSV file vary depending on the type of migration. For more information, see CSV files for mialbox migrations (https://go.microsoft.com/fwlink/p/?LinkId=624779).

Use the following format for the value of this parameter: ([System.IO.File]::ReadAllBytes(\<path of the CSV migration file\>)). The following is an example: CSVData:([System.IO.File]::ReadAllBytes("C:\\Users\\Administrator\\Desktop\\MigrationBatch\_1.csv"))
...
The PrimaryOnly switch specifies that only primary mailboxes are migrated from the source organization to the target organization for the users specified in the migration batch. Archive mailboxes aren't migrated when you use this switch.

This switch can only be used for local moves and remote move migrations. Also, this switch applies only to users that have archive mailboxes. If the users don't have archive mailboxes, don't use this switch.
